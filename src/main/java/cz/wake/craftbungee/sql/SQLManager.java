...
public class SQLManager {
    ...
    public final void addVoteToken3(final String p, final int votetokens) {
        Main.getInstance().getProxy().getScheduler().runAsync(Main.getInstance(), () -> {
            Connection conn = null;
            PreparedStatement ps = null;
            try {
                conn = pool.getConnection();
                ps = conn.prepareStatement("UPDATE minigames.player_profile SET vote_tokens_3 = vote_tokens_3 + " + votetokens + " WHERE nick = ?;");
                ps.setString(1, p);
                ps.executeUpdate();
            } catch (Exception e) {
                e.printStackTrace();
